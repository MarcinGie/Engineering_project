...
% na pełnej rozdzielczości. Doprecyzowanie wysokości progu i ostateczna
...
for eee=1:119
    ...
    for j=1:a 
        if R(eee,1).W_O(j,1).bz==0 
        	% przeliczenie BB z układu mniejszej rozdzielczości na większą
            ...
            D=cat(1,(reshape(Ow_hsv(:,:,1),1,(aa*bb))),(reshape(Ow_hsv(:,:,2),1,(aa*bb))),(reshape(Ow_hsv(:,:,3),1,(aa*bb)))); % wektor wejściowy do sieci
            k1k2=sim(net2,D); %ładowanie macierzy do sieci
            K1=reshape(k1k2(1,:),aa,bb); %obraz odpowiedzi węzła K1 sieci 
