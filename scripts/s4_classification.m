...
for eee=1:14
    ...
    if R(eee,1).t==1
        ...
        f=1;
        x=0;
            if R(eee,1).TZ(1,1).bz==0
                [e,f]=size(R(eee,1).TZ(1,1).Sign);
                ZNAK=R(eee,1).TZ(1,1).masked;
                [c,d]=size(ZNAK);
                k=1;
                for j=30:-3:-30
                    n=num2str(j);
                ...
                ORYG=R(eee,1).O;
                figure(eee);subplot(1,4,1+x);imshow(ORYG);
                title('fragment obrazu oryginalnego');
                K1=imcrop(R(eee,1).K1_OST, R(eee,1).TZ(1,1).bb);
                figure(eee);subplot(1,4,2+x);imshow(K1);
                title('fragment progowania pocz?tkowego');
                figure(eee);subplot(1,4,3+x);imshow(ZNAK);
                title('ostateczny obraz progowania po prostowaniu');
                n=num2str(R(eee,1).TZ(1,1).kat);
                nazwa=strcat(R(eee,1).TZ(1,1).n,n,koncowka);
                S=imread([sciezka_wzorce nazwa]);
                figure(eee);subplot(1,4,4+x);imshow(S);
